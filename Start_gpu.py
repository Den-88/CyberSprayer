...
def detect_green(frame):
    # Преобразование в цветовое пространство HSV с использованием UMat для ускорения
    frame_umat = cv2.UMat(frame)  # Использование OpenCL для ускорения
...
def main():
    ...
    while True:
        ...
        # Переводим кадр в UMat для работы с GPU
        frame_umat = cv2.UMat(frame)

        green_detected, green_ratio = detect_green(frame_umat)
        ...
        # Переводим обратно на CPU для отображения
